...
```{r}
write.csv(clinic, "/Users/lantrieu1964/Documents/GitHub/qbio_490_LanTrieu/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/lantrieu1964/Documents/GitHub/qbio_490_LanTrieu/analysis_data/brca_clinical_data.csv")
```
...
View the structure of the data as well as the first few lines. What two commands should you use? str() and head()
Answer the following questions based on the output of those commands: 
# 1. How many rows are there? How many columns? 89568 rows, 141 columns
# 2. What is the data type of $gender? "NULL"
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? 50
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? 42
...
```{r Exercise 3.3}
# write code here
You can use $vital_status or [vital_status]
```
...
```{r Exercise 4.1}
# create plot here
plot(initial_pathologic_dx_year$x, lymph_nodes_examined_count$y, main="Scatter Plot", xlab="Initial Pathologic dx Year", ylab="Number of Lymph Nodes Examined", col="blue", pch=19)
```
...
```{r Exercise 4.2}
# create plot here
boxplot(age_at_last_follow_up$numeric)
```
...
```{r}
no_middle_clinic <- subset(clinic, age_at_diagnosis < 30 | age_at_diagnosis > 60)
```
...
cleaned_clinic <- clinic[!is.na(clinic$age_at_diagnosis), ]
```
...
# 4. What age group appears to have the worst rate of survival? Old, though the rate of survival for "young" is worse after about 3000 days.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? Approximately 0.3

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? The p-value is 0.2, which is less than the significance value of 0.05. It is therefore not statistically significant and indicates that there is no sufficient evidence to prove that there is a relationship between age and survival probability.
